...
- 当前阶段：S1区域/病变标注监督审计完成；S1a真实录入/裁决被硬门锁定，等待书面确认八个医学定义及八个复核参数；G0正常/异常图像门控已获负责人确认并冻结配置，进入实现阶段，尚未训练；S1b几何可靠性将在S1a通过后另行预登记
...
| G0 正常/异常图像门控 | 已冻结，进入实现 | [G0预登记](../project/G0患者级正常异常图像门控实验预登记_2026-08-03.md)与[ADR-013](../decisions/ADR-013-以独立正常异常图像门控重开分层融合.md) | 配置SHA `4a06e6...ed72`；当前无权重、OOF或MLflow run |
