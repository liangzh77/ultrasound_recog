...
- 当前阶段：E3五折完成但总体和类别安全失败；A2仍无合格图像模型，转入28类区域/病变标注监督可用性审计
...
| 区域/病变监督审计 | 待启动 | [ADR-009](../decisions/ADR-009-E3未通过总体及类别安全门槛并转向标注监督审计.md) | 先审计28类覆盖、共现、折分布和疾病捷径，再决定分割/多任务路线 |
