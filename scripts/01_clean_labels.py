...
遍历 data/膝关节已标注/ 下所有 JSON 标注文件：
1. 应用 label_mapping.py 中的修复规则
2. 输出清洗后的数据到 data/cleaned/（保持原目录结构）
3. 图片通过软链接/复制关联
4. 生成清洗报告 data/clean_report.json

用法:
    python scripts/01_clean_labels.py [--dry-run]
"""
...
def main():
    ...
    # 保存报告
    report_path = ROOT / "data" / "clean_report.json"
    ...
    if not args.dry_run:
        ...
        cats_file = ROOT / "data" / "category_mapping.json"
