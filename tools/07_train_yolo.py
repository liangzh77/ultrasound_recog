...
用法:
    python tools/07_train_yolo.py [--resume]
    python tools/07_train_yolo.py --predict <image_path>
"""
...
from src.common_paths import LEGACY_CONFIGS_DIR, LEGACY_YOLO_RUN_DIR, YOLO11M_SEG_PATH
from src.label_mapping import LABEL_PREFIX_TO_DISEASE

CONFIG_PATH = LEGACY_CONFIGS_DIR / "yolo_seg.yaml"
...
def train(resume: bool = False):
    ...
    if resume:
        model = YOLO(str(LEGACY_YOLO_RUN_DIR / "knee_v1" / "weights" / "last.pt"))
    else:
        model = YOLO(str(YOLO11M_SEG_PATH))
    ...
    model.train(
        ...
        device=0,
        workers=2,
        project=str(LEGACY_YOLO_RUN_DIR),
        name="knee_v1",
        exist_ok=resume,
        # 类别不平衡
        cls=0.5,
...
def predict(image_path: str):
    ...
    best_weight = LEGACY_YOLO_RUN_DIR / "knee_v1" / "weights" / "best.pt"
    ...
    results = model.predict(
        ...
        save=True,
        save_txt=True,
        project=str(LEGACY_YOLO_RUN_DIR / "predict"),
        conf=0.25,
        iou=0.5,
    )
