...
用法:
    python tools/11_evaluate.py --method yolo
    python tools/11_evaluate.py --method nnunet
    python tools/11_evaluate.py --method medsam --sam-weights path/to/medsam_vit_b.pth
    python tools/11_evaluate.py --method classifier
    python tools/11_evaluate.py --compare  # 对比所有方案
"""
...
RESULTS_DIR = LEGACY_EVALUATION_DIR
...
def evaluate_yolo():
    ...
    best_weight = LEGACY_YOLO_RUN_DIR / "knee_v2" / "weights" / "best.pt"
    if not best_weight.exists():
        print(f"错误: YOLO 模型权重不存在: {best_weight}")
        return None

    model = YOLO(str(best_weight))
    config_path = LEGACY_CONFIGS_DIR / "yolo_seg_merged.yaml"
...
def evaluate_nnunet():
    ...
    val_pred_dir = (
        LEGACY_NNUNET_RESULTS_DIR / "Dataset001_Knee"
        / "nnUNetTrainer__nnUNetPlans__2d" / "fold_0" / "validation"
    )
    gt_dir = NNUNET_DIR / "Dataset001_Knee" / "labelsTr"
    ...
    # 加载类别名
    dataset_json = NNUNET_DIR / "Dataset001_Knee" / "dataset.json"
