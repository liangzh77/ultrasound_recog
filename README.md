...
- G0 患者级正常/异常图像门控预登记草案：
  `docs/project/G0患者级正常异常图像门控实验预登记草案_2026-08-03.md`
  — 在不复活失败六分类模型、不读取 Excel 或 2026 标签的前提下，提出单一二分类患者 MIL 候选及固定门槛；当前等待项目负责人确认，尚未训练。
...
- 正常/异常图像门控决策草案：
  ...
  — 提议用独立 G0 二分类任务补齐原分层架构；不改变 E1～E3 阴性结论，只有 G0 全部门槛通过后才允许另行预登记融合。
- 临床特征与标注规范化决策：
